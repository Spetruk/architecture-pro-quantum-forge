#!/usr/bin/env python3
"""
Создание векторного индекса для базы знаний из Task5/knowledge_base
"""
...
class KnowledgeBaseIndexer:
    def __init__(self, knowledge_base_path: str | None = None):
        ...
        self.knowledge_base_path = (
            Path(knowledge_base_path) if knowledge_base_path else script_dir.parent / "knowledge_base"
        )
    ...
    def create_vector_index(self, persist_directory: str | None = None) -> Chroma:
        print("🔍 Создание векторного индекса (chroma_db)...")
        target_dir = Path(persist_directory) if persist_directory else (self.base_dir.parent / "chroma_db")
...
def main():
    print("🚀 Запуск создания векторного индекса (Task7)")
    print("=" * 50)
    start = time.time()
    ...
    if not indexer.chunks:
        print("⚠️ Нет данных для индексации (0 чанков). Проверьте knowledge_base.")
        return
